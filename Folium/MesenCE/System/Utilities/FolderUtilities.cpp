...
{
	string folder = CombinePath(GetHomeFolder(), "hd_packs");
	CreateFolder(folder);
	return folder;
}
...
string FolderUtilities::GetSaveStateFolder()
{
	...
	if(_saveStateFolderOverride.empty()) {
		folder = CombinePath(GetHomeFolder(), "save_states");
	} else {
...
{
	string folder = CombinePath(GetHomeFolder(), "recent_games");
	CreateFolder(folder);
	return folder;
}
...
void FolderUtilities::CreateFolder(string folder)
{
	...
	fs::create_directory(fs::path(folder), errorCode);
}
...
vector<string> FolderUtilities::GetFolders(string rootFolder)
{
	...
	std::error_code errorCode;
	if(!fs::is_directory(fs::path(rootFolder), errorCode)) {
		return folders;
	}

	for(fs::recursive_directory_iterator i(fs::path(rootFolder).string()), end; i != end; i++) {
		if(i.depth() > 1) {
...
vector<string> FolderUtilities::GetFilesInFolder(string rootFolder, std::unordered_set<string> extensions, bool recursive, int maxDepth)
{
	...
	if(recursive) {
		...
		for(fs::directory_iterator i(fs::path(rootFolder).string()), end; i != end; i++) {
			string extension = i->path().extension().string();
			std::transform(extension.begin(), extension.end(), extension.begin(), ::tolower);
			if(extensions.empty() || extensions.find(extension) != extensions.end()) {
				files.push_back(i->path().string());
