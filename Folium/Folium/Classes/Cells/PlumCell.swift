...
class PlumCell : GameCell {
    var game: PlumGame? = nil
    override func configureCell<T>(with game: T, controller: GamesController) {
        self.game = game as? PlumGame
        guard let game: PlumGame = self.game else {
            return
        }
...
extension PlumCell : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
