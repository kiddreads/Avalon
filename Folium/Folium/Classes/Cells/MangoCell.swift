...
class MangoCell : GameCell {
    var game: MangoGame? = nil
    override func configureCell<T>(with game: T, controller: GamesController) {
        self.game = game as? MangoGame
        guard let game: MangoGame = self.game else {
            return
        }
...
extension MangoCell : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
