...
class DurianCell : GameCell {
    var game: DurianGame? = nil
    override func configureCell<T>(with game: T, controller: GamesController) {
        self.game = game as? DurianGame
        guard let game: DurianGame = self.game else {
            return
        }
...
extension DurianCell : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
