...
class LycheeController : ControlsController {
    override func viewDidLoad() {
        ...
        let northConfiguration: UIButton.Configuration = .configuration(.large, .capsule, nil, "X", .large)
        ...
        let westConfiguration: UIButton.Configuration = .configuration(.large, .capsule, nil, "Y", .large)
        ...
        switch system {
        case .lychee:
            configureConstraintsForLychee()
            reconfigureConstraintsForLychee()
        default:
            break
        }
    ...
    override func viewWillLayoutSubviews() {
        ...
        guard let lycheeGame: LycheeGame = game as? LycheeGame else {
            return
        }
        
        _ = Task {
            if await lycheeGame.lycheeSystem.running {
                return
            }
            ...
            await lycheeGame.lycheeSystem.setContext(context: Unmanaged.passUnretained(self).toOpaque())
            
            lycheeGame.lycheeSystem.audioBuffer { context, pointer, samples in
            ...
            lycheeGame.lycheeSystem.videoBuffer { context, pointer, _ in
                ...
                guard let imageView: UIImageView = viewController.primaryRenderingView as? UIImageView,
                      ...
                      let game: LycheeGame = viewController.game as? LycheeGame else {
                    return
                }
                
                Task { @MainActor in
                    let height: Int32 = await game.lycheeSystem.framebufferHeight
                    let width: Int32 = await game.lycheeSystem.framebufferWidth
            ...
            await lycheeGame.lycheeSystem.start()
        }
    }
    
    func press(button: LycheeButton) {
        guard let lycheeGame: LycheeGame = game as? LycheeGame else {
            return
        }
        
        press(button: button, using: lycheeGame.lycheeSystem)
    }
    
    func release(button: LycheeButton) {
        guard let lycheeGame: LycheeGame = game as? LycheeGame else {
            return
        }
        
        release(button: button, using: lycheeGame.lycheeSystem)
    }
}

extension LycheeController {
    func reconfigureConstraintsForLychee() {
