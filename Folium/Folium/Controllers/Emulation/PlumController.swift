...
class PlumController : ControlsController {
        ...
        settingsButton = .button(with: settingsConfiguration,
                                 actions: ({ _ in }, { _ in }), UIMenu(preferredElementSize: .medium, children: [
                                    UIDeferredMenuElement.uncached { completion in
                                        guard let plumGame: PlumGame = self.game as? PlumGame else {
                                            completion([])
                                            return
                                        }
                                        ...
                                        Task {
                                            completion([
                                                UIAction.async(title: await plumGame.plumSystem.paused ? "Resume" : "Pause",
                                                               image: UIImage(systemName: await plumGame.plumSystem.paused ? "play.fill" : "pause.fill")) { action in
                                                                   if await plumGame.plumSystem.paused {
                                                                       await plumGame.plumSystem.set(change: true, isPaused: false)
                                                                   } else {
                                                                       await plumGame.plumSystem.set(change: true, isPaused: true)
                                                                   }
                                                },
                                                UIAction.async(title: "Stop & Exit", image: UIImage(systemName: "stop.fill"), attributes: .destructive) { action in
                                                    await plumGame.plumSystem.stop()
    ...
    override func viewWillLayoutSubviews() {
        ...
        guard let plumGame: PlumGame = game as? PlumGame else {
            return
        }
        
        _ = Task {
            if await plumGame.plumSystem.running {
                return
            }
            ...
            await plumGame.plumSystem.setContext(context: Unmanaged.passUnretained(self).toOpaque())
            
            plumGame.plumSystem.videoBuffer { context, pointer in
                guard let context, let pointer else {
                    return
                }
                
                let viewController: PlumController = Unmanaged<PlumController>.fromOpaque(context).takeUnretainedValue()
                ...
                guard let imageView: UIImageView = viewController.primaryRenderingView as? UIImageView,
                      ...
                      let game: PlumGame = viewController.game as? PlumGame else {
                    return
                }
                ...
                Task { @MainActor in
                    ...
                    viewController.send(frame: image, system: .plum)
                }
            }
            
            await plumGame.plumSystem.start()
        }
    }
    ...
    func press(button: PlumButton) {
        ...
        press(button: button, using: plumGame.plumSystem)
    }
    ...
    func release(button: PlumButton) {
        ...
        release(button: button, using: plumGame.plumSystem)
    }
    
    override nonisolated func receive(button: PlumButton, pressed: Bool) {
        guard let plumGame: PlumGame = game as? PlumGame else {
            return
        }
        
        if pressed {
            press(button: button, index: 1, using: plumGame.plumSystem)
        } else {
            release(button: button, index: 1, using: plumGame.plumSystem)
        }
    }
}

extension PlumController {
