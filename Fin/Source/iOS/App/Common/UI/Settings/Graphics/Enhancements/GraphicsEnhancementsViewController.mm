...
- (void)viewWillAppear:(BOOL)animated {
  ...
  NSString* postShader = [DOLConfigBridge postProcessingShader];
  self.postProcessingCell.choiceSettingLabel.text =
      (postShader.length > 0) ? postShader : DOLCoreLocalizedString(@"None");
}
    ...
    case 0:
      switch (indexPath.row) {
        ...
        case 8:
          ...
          break;
        case 9:
          message = @"Apply a custom post-processing shader to the final image. Place .glsl files in the Shaders "
                    "folder in Dolphin's user directory. Shaders must use Dolphin's post-processing API.";
          break;
      }
      break;
  }
