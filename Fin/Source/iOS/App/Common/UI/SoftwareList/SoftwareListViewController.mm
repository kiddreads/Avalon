...
- (void)presentEmulation {
  EmulationSwiftUIHostingController* emulationVC = [[EmulationSwiftUIHostingController alloc] initWithBootParameter:_bootParameter];
  emulationVC.modalPresentationStyle = UIModalPresentationFullScreen;
  [self presentViewController:emulationVC animated:YES completion:nil];
}
...
{
  if ([segue.identifier isEqualToString:@"wiiUpdate"]) {
