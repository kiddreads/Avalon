...
int Presenter::GetSourceWidth() const
{
  if (m_xfb_entry && m_xfb_rect.GetWidth() > 0)
    return m_xfb_rect.GetWidth();
  return 640;
}

int Presenter::GetSourceHeight() const
{
  if (m_xfb_entry && m_xfb_rect.GetHeight() > 0)
    return m_xfb_rect.GetHeight();
  return 480;
}
...
void Presenter::SetBackbuffer(SurfaceInfo info)
{
  ...
  if (m_onscreen_ui)
  {
    ...
    // ImGui pipeline is compiled for a specific backbuffer format (8-bit sRGB vs RGBA16F linear).
    // When the format changes (e.g. HDR toggled, or surface recreated), recompile so OSD/save
    // state popup and other UI no longer render as corrupted.
    if (format_changed)
...
void Presenter::UpdateDrawRectangle()
{
  ...
  // The rendering window size. Skip when backbuffer is invalid to avoid NaN and invalid
  // target rectangle (which can trigger "Contradictory frame constraints" / "Invalid frame
  // dimension" on iOS when post-processing or layout use these values).
  const float win_width = static_cast<float>(m_backbuffer_width);
  const float win_height = static_cast<float>(m_backbuffer_height);
  if (win_width <= 0 || win_height <= 0 || !std::isfinite(win_width) || !std::isfinite(win_height))
  {
    m_target_rectangle = {0, 0, 0, 0};
    return;
  }
