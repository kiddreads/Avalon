  ...
  m_gpu_mainloop.Run(
      [this] {
        ...
        else
        {
          ...
          // Check and update CP status at the start of each iteration so CPU-side
          // interrupt timing closely matches upstream behavior.
          while (true)
          {
            command_processor.SetCPStatusFromGPU();

            if (command_processor.IsInterruptWaiting())
              break;

            if (!fifo.bFF_GPReadEnable.load(std::memory_order_relaxed))
              break;

            const u32 cp_distance = fifo.CPReadWriteDistance.load(std::memory_order_relaxed);
            if (!cp_distance)
              break;

            if (AtBreakpoint(m_system))
              break;
            ...
            const s32 distance = static_cast<s32>(cp_distance) - GPFifo::GATHER_PIPE_SIZE;
            ...
            {
              fifo.SafeCPReadPointer.store(readPtr, std::memory_order_relaxed);
            }
