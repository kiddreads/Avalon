...
struct BackendInfo
{
  ...
  bool bSupportsGPUVertexDecode = false;
...
struct VideoConfig final
{
  ...
  // Metal Drawable
  bool bScaleDrawableWithInternalRes = true;

  // Native Video Decode (iOS)
  bool bUseNativeVideoDecode = true;

  // MetalFX Upscaling
  bool bMetalFXUpscaling = false;
  int iMetalFXOutputWidth = 0;
  int iMetalFXOutputHeight = 0;
  bool bMetalFXOutput125xInternal = false;
  int iMetalFXInternalScale = 50;
  bool bUseGPUVertexDecode = true;
  ...
  struct
  {
    ...
    // Exposure for HDR/tone mapping (1.0 = no adjustment). Used by post-processing tone mapping.
