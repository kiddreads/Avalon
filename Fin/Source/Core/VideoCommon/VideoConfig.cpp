...
void VideoConfig::Refresh()
{
  ...
  bUseNativeVideoDecode = Config::Get(Config::GFX_USE_NATIVE_VIDEO_DECODE);
  bMetalFXUpscaling = Config::Get(Config::GFX_MTL_FX_UPSCALING);
  bUseGPUVertexDecode = Config::Get(Config::GFX_MTL_GPU_VERTEX_DECODE);
  iMetalFXOutputWidth = Config::Get(Config::GFX_MTL_FX_OUTPUT_WIDTH);
  iMetalFXOutputHeight = Config::Get(Config::GFX_MTL_FX_OUTPUT_HEIGHT);
  bMetalFXOutput125xInternal =
      Config::Get(Config::GFX_MTL_FX_OUTPUT_125X_INTERNAL);
  iMetalFXInternalScale = Config::Get(Config::GFX_MTL_FX_INTERNAL_SCALE);
  ...
  color_correction.fExposure = Config::Get(Config::GFX_CC_EXPOSURE);
...
void VideoConfig::VerifyValidity()
{
  ...
  if (bUseGPUVertexDecode && !g_backend_info.bSupportsGPUVertexDecode)
    bUseGPUVertexDecode = false;
}
...
void CheckForConfigChanges()
{
  ...
  const bool old_scale_drawable_with_internal_res =
      g_ActiveConfig.bScaleDrawableWithInternalRes;
  const bool old_metal_fx_output_125x = g_ActiveConfig.bMetalFXOutput125xInternal;
  ...
  if (old_scale_drawable_with_internal_res != g_ActiveConfig.bScaleDrawableWithInternalRes)
    changed_bits |= CONFIG_CHANGE_BIT_TARGET_SIZE;
  if (old_metal_fx_output_125x != g_ActiveConfig.bMetalFXOutput125xInternal)
    changed_bits |= CONFIG_CHANGE_BIT_TARGET_SIZE;
