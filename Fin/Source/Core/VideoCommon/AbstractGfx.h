...
public:
  ...
  virtual std::unique_ptr<AbstractShader> CreateShaderFromBinary(ShaderStage stage,
                                                                 ...
                                                                 std::string_view name = "") = 0;
  // Slang: translate SPIR-V to backend shader source (e.g. MSL). Returns empty if not supported.
  virtual std::optional<std::string> TranslateSlangSPIRVToMSL(ShaderStage stage,
