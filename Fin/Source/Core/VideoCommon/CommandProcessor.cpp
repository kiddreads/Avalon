...
{
  const bool bp_enable = m_fifo.bFF_BPEnable.load(std::memory_order_relaxed) != 0;
  const u32 cp_breakpoint = m_fifo.CPBreakpoint.load(std::memory_order_relaxed);
  const u32 cp_read_pointer = m_fifo.CPReadPointer.load(std::memory_order_relaxed);
  bool breakpoint = m_fifo.bFF_Breakpoint.load(std::memory_order_relaxed);

  if (bp_enable && cp_breakpoint == cp_read_pointer)
  {
    if (!breakpoint)
    {
      DEBUG_LOG_FMT(COMMANDPROCESSOR, "Hit breakpoint at {}", cp_read_pointer);
  ...
  {
    DEBUG_LOG_FMT(COMMANDPROCESSOR, "Cleared breakpoint at {}", cp_read_pointer);
    m_fifo.bFF_Breakpoint.store(0, std::memory_order_relaxed);
    breakpoint = false;
  }

  const u32 cp_read_write_distance =
      m_fifo.CPReadWriteDistance.load(std::memory_order_relaxed);
  const bool hi_watermark = cp_read_write_distance > m_fifo.CPHiWatermark;
  const bool lo_watermark = cp_read_write_distance < m_fifo.CPLoWatermark;
  ...
  const bool bp_int = breakpoint && m_fifo.bFF_BPInt.load(std::memory_order_relaxed);
  const bool ovf_int =
      hi_watermark && m_fifo.bFF_HiWatermarkInt.load(std::memory_order_relaxed);
  const bool undf_int =
      lo_watermark && m_fifo.bFF_LoWatermarkInt.load(std::memory_order_relaxed);

  const bool interrupt =
      (bp_int || ovf_int || undf_int) && m_cp_ctrl_reg.GPReadEnable;
