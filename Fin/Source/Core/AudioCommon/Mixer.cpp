...
void Mixer::MixerFifo::Mix(s16* samples, std::size_t num_samples)
{
  ...
  const std::size_t computed_granules = (buffer_size_samples) / (GRANULE_SIZE >> 1);
#if defined(IPHONEOS)
  constexpr std::size_t kMinGranules = 12;
#else
  constexpr std::size_t kMinGranules = 4;
#endif
  const std::size_t buffer_size_granules =
      std::clamp(computed_granules, kMinGranules, static_cast<std::size_t>(MAX_GRANULE_QUEUE_SIZE));
...
float Mixer::GetBufferFillRatio() const
{
  // RT-safe: delegate to MixerFifo method
  return m_dma_mixer.GetFillRatio();
}
...
float Mixer::MixerFifo::GetFillRatio() const
{
  // RT-safe: read atomics only, no locks
  ...
  const std::size_t target_size = m_granule_queue_size.load(std::memory_order_relaxed);
  
  // Calculate fill: (head - tail) mod queue_size
  const std::size_t fill = (head - tail) & GRANULE_QUEUE_MASK;
  
  // Return ratio (clamped to [0, 1])
  if (target_size == 0)
    return 0.5f;  // Default to middle if target not set
  
  const float ratio = static_cast<float>(fill) / static_cast<float>(target_size);
...
bool Mixer::MixerFifo::Dequeue(Granule* granule)
{
  ...
  if (next_tail == head)
  {
    // Fill-audio-gaps: single path here (no other fill logic elsewhere). Repeat last stretch when underrun.
    // Only fill gaps when running to prevent stutter on pause.
    // RT-safe: use atomic flag instead of Core::GetState
    const bool is_running = m_mixer->m_is_running.load(std::memory_order_acquire);
