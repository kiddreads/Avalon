...
public:
  ...
  // Bell Audio: Get buffer fill ratio (0.0 = empty, 1.0 = full) for audio-driven frame pacing
  // Returns fill ratio of DMA mixer (main game audio). RT-safe, lock-free.
  float GetBufferFillRatio() const;
...
private:
  ...
  // RT-safe flag: set by host when emulation starts/stops (replaces Core::GetState in Dequeue)
  std::atomic<bool> m_is_running{false};

  Config::ConfigChangedCallbackID m_config_changed_callback_id;

public:
  // Called by host when emulation starts/stops (RT-safe alternative to Core::GetState)
  void SetIsRunning(bool running) { m_is_running.store(running, std::memory_order_release); }
};
