...
#ifdef IPHONEOS
#ifdef BACKEND_BELL
  else if (backend == BACKEND_BELL && CoreAudioSound::IsValid())
#else
  else if (backend == "Bell" && CoreAudioSound::IsValid())
#endif
  {
    // Bell backend uses CoreAudioSound with Bell Audio features enabled.
    // Config flags are set before boot in EmulationCoordinator to avoid
    // Config::SetBase calls during BootCore which can cause deadlocks.
    return std::make_unique<CoreAudioSound>();
  }
#endif
  return {};
...
#ifdef IPHONEOS
#ifdef BACKEND_BELL
    backends.emplace_back(BACKEND_BELL);
#else
    backends.emplace_back("Bell");
#endif
#endif
  }
...
void SetSoundStreamRunning(Core::System& system, bool running)
{
  ...
  // RT-safe: set atomic flag for Mixer (replaces Core::GetState in Dequeue)
  Mixer* mixer = sound_stream->GetMixer();
  if (mixer)
    mixer->SetIsRunning(running);
