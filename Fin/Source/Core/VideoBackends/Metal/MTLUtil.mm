...
void Metal::Util::PopulateBackendInfo(BackendInfo* backend_info)
{
  ...
  backend_info->bSupportsGPUVertexDecode = false;  // Metal vertex decode shaders removed
...
#else
  // iOS: set in InitBackendInfo from WindowSystemInfo::display_supports_edr (app provides via UIKit).
                                                             ...
                                                             std::string_view source)
{
  ...
  if (!code.has_value())
  {
    ERROR_LOG_FMT(VIDEO, "Failed to compile GLSL to SPIR-V for {} shader", 
                  stage == ShaderStage::Vertex ? "vertex" : 
                  stage == ShaderStage::Pixel ? "pixel" : "compute");
    ERROR_LOG_FMT(VIDEO, "GLSL source (first 500 chars): {}", 
                  full_source.substr(0, std::min<size_t>(500, full_source.size())));
    return std::nullopt;
  }

  // Keep a copy for fragment shaders so we can retry without framebuffer fetch if needed
  std::optional<SPIRV::CodeVector> pixel_spirv_copy;
  if (stage == ShaderStage::Pixel)
    pixel_spirv_copy = *code;
  ...
  static const spirv_cross::MSLResourceBinding resource_bindings[] = {
      ...
      MakeResourceBinding(spv::ExecutionModelFragment,  0, 0, 0, 0, 0), // ps/ubo
      MakeResourceBinding(spv::ExecutionModelFragment,  0, 1, 0, 0, 0), // ps/ubo
      MakeResourceBinding(spv::ExecutionModelFragment,  0, 3, 0, 0, 0), // ps/ubo
      MakeResourceBinding(spv::ExecutionModelFragment,  2, 0, 2, 2, 0), // ps/ssbo (palette/texel buffer)
      MakeResourceBinding(spv::ExecutionModelFragment,  1, 0, 0, 16, 0), // ps/input attachment (framebuffer fetch)
      MakeResourceBinding(spv::ExecutionModelGLCompute, 0, 0, 0, 0, 0), // cs/ubo
      MakeResourceBinding(spv::ExecutionModelGLCompute, 0, 1, 1, 0, 0), // cs/ubo
  ...
  // Disable framebuffer fetch for fragment shaders by default (post-processing shaders don't use it)
  // We'll retry with it disabled if compilation fails
  ...
  {
    // Retry fragment shaders WITH framebuffer fetch (for Dolphin internal shaders that need it)
    if (stage == ShaderStage::Pixel && pixel_spirv_copy.has_value())
    {
      WARN_LOG_FMT(VIDEO, "SPIR-V to MSL compilation failed (without framebuffer fetch): {}", e.what());
      WARN_LOG_FMT(VIDEO, "Retrying with framebuffer fetch enabled...");
      try
      {
        spirv_cross::CompilerMSL::Options fallback_options = options;
        fallback_options.use_framebuffer_fetch_subpasses = true;
        spirv_cross::CompilerMSL compiler2(std::move(*pixel_spirv_copy));
        compiler2.set_msl_options(fallback_options);
        for (const auto& binding : resource_bindings)
          compiler2.add_msl_resource_binding(binding);
        for (u32 i = 0; i < VideoCommon::MAX_PIXEL_SHADER_SAMPLERS; i++)
          compiler2.add_msl_resource_binding(
              MakeResourceBinding(spv::ExecutionModelFragment, 1, i, 0, i, i));
        compiled = compiler2.compile();
        INFO_LOG_FMT(VIDEO, "Successfully compiled with framebuffer fetch enabled");
      }
      catch (const std::exception& e2)
      {
        ERROR_LOG_FMT(VIDEO, "SPIR-V to MSL compilation failed (with framebuffer fetch): {}", e2.what());
        throw;
      }
    }
    else
    {
      ERROR_LOG_FMT(VIDEO, "SPIR-V to MSL compilation failed: {}", e.what());
      throw;
    }
  }
...
Metal::Util::TranslateSlangSPIRVToMSL(ShaderStage stage, const std::vector<u32>& spirv)
{
  // For Slang we start from SPIR-V produced by Vulkan GLSL; preserve the original bindings.
  spirv_cross::CompilerMSL::Options options;
#if TARGET_OS_OSX
  options.platform = spirv_cross::CompilerMSL::Options::macOS;
#elif TARGET_OS_IOS
  ...
  options.set_msl_version(2, 3);
  // For slang, avoid forcing framebuffer fetch; shaders control their own IO.
  options.use_framebuffer_fetch_subpasses = false;
  ...
  {
    ERROR_LOG_FMT(VIDEO, "Slang SPIR-V -> MSL failed: {}", e.what());
    return std::nullopt;
  }
