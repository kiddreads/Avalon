...
static NSString* GetName(Metal::StateTracker::UploadBuffer buffer)
{
  ...
  switch (buffer)
  {
    ...
    case Metal::StateTracker::UploadBuffer::RawVertexInput: return @"Raw Vertex Input";
...
id<MTLBuffer> Metal::StateTracker::GetVertexBuffer() const
{
  const BufferPair& buffer = m_upload_buffers[static_cast<int>(UploadBuffer::Vertex)];
  return (m_manual_buffer_upload && buffer.gpubuffer) ? buffer.gpubuffer.Get() : buffer.cpubuffer.Get();
}
...
void Metal::StateTracker::SetPipeline(const ComputePipeline* pipe)
{
  if (pipe != m_state.compute_pipeline)
  {
    ...
    m_flags.has_pipeline = false;
    m_state.compute_buffers.fill(nil);
  }
}
...
void Metal::StateTracker::SetComputeBuffer(id<MTLBuffer> buffer, size_t offset, u32 index)
{
  ASSERT(index < StateTracker::State::MAX_COMPUTE_BUFFERS);
  m_state.compute_buffers[index] = buffer;
  m_state.compute_buffer_offsets[index] = offset;
  m_flags.has_utility_vs_uniform = false;
  m_flags.has_texel_buffer = false;
}
...
void Metal::StateTracker::PrepareCompute()
{
  ...
  for (u32 i = 0; i < StateTracker::State::MAX_COMPUTE_BUFFERS; i++)
  {
    if (!pipe->UsesBuffer(i))
      continue;
    if (m_state.compute_buffers[i])
    {
      [enc setBuffer:m_state.compute_buffers[i] offset:m_state.compute_buffer_offsets[i] atIndex:i];
    }
    else if (i == 0 && m_state.utility_uniform)
    {
      m_flags.has_utility_vs_uniform = true;
      [enc setBytes:m_state.utility_uniform.get() length:m_state.utility_uniform_size atIndex:0];
    }
    else if (i == 2 && m_state.texels)
    {
      m_flags.has_texel_buffer = true;
