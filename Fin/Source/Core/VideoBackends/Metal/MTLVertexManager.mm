...
void Metal::VertexManager::ResetBuffer(u32 vertex_stride)
{
  ...
  // Stash GPU offset of current vertex allocation for DecodeVerticesOnGPU
  m_vertex_buffer_base_offset = vertex.second;
  ...
  m_vertex_buffer_base_offset += m_vertex_offset;
...
bool Metal::VertexManager::DecodeVerticesOnGPU(VertexLoaderBase* loader, const u8* src, int count)
{
  (void)loader;
  (void)src;
  (void)count;
  return false;  // Metal vertex decode shaders removed; use CPU vertex loader only
}
