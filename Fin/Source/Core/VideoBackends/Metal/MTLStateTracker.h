...
public:
  ...
  /// Set buffer for compute pipeline at index (e.g. vertex decode: 0=raw, 1=output, 2..13=arrays, 14=uniform).
  void SetComputeBuffer(id<MTLBuffer> buffer, size_t offset, u32 index);
  ...
  /// Returns the Vertex upload buffer (for GPU vertex decode output). Valid after Preallocate(Vertex).
...
private:
  ...
  struct State
  {
    ...
    std::array<id<MTLBuffer>, MAX_COMPUTE_BUFFERS> compute_buffers = {};
    std::array<size_t, MAX_COMPUTE_BUFFERS> compute_buffer_offsets = {};
