    ...
    : m_controller_id(controller_id)
{
  ...
  if ([[CHHapticEngine capabilitiesForHardware] supportsHaptics])
  {
    ...
    NSDictionary* payload = @{
      ...
      @"location": @"Touchscreen.mm:Touchscreen()", @"message": @"Creating Motor for Touchscreen",
      @"data": @{ @"controller_id": @(m_controller_id), @"onMainThread": @([NSThread isMainThread]) },
    ...
    if (json) {
      ...
      [line appendBytes:"\n" length:1];
      if (![[NSFileManager defaultManager] fileExistsAtPath:path]) [[NSData data] writeToFile:path atomically:NO];
      NSFileHandle* f = [NSFileHandle fileHandleForUpdatingAtPath:path];
