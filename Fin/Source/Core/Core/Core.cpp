...
#ifdef __APPLE__
  // Boost CPU thread priority to prevent starvation by audio IO and system threads.
  // On iOS the default priority is too low and the cached interpreter gets < 1% CPU time.
  // Use QOS_CLASS_USER_INTERACTIVE — the highest non-realtime QoS available to apps.
