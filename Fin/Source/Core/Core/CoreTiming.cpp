...
#ifdef IPHONEOS
  // Bell Audio: Driven pacing disabled — the audio callback handles buffer management
  // via underrun protection and dynamic rate correction. Running a proportional
  // controller here on every Throttle() tick added measurable CPU overhead and
  // conflicted with the callback-side correction, causing lag and instability.
#endif
