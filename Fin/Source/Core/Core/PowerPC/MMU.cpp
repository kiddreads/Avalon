...
T MMU::ReadFromHardware(u32 em_address)
{
  ...
  if (flag == XCheckTLBFlag::Read && (em_address & 0xF8000000) == 0x08000000) [[unlikely]]
  {
  ...
  if (m_memory.GetRAM() && (em_address & 0xF8000000) == 0x00000000) [[likely]]
  {
  ...
  if (m_memory.GetEXRAM() && (em_address >> 28) == 0x1 &&
      (em_address & 0x0FFFFFFF) < m_memory.GetExRamSizeReal()) [[likely]]
  {
    T value;
  ...
  {
    static bool s_showed_read_warning = false;
    if (!s_showed_read_warning)
    {
      s_showed_read_warning = true;
      Core::DisplayMessage("\u26A0 Unresolvable read address (ignored)", 4000);
    }
  }
  return 0;
}
...
void MMU::WriteToHardware(u32 em_address, const u32 data, const u32 size)
{
  ...
  {
    static bool s_showed_write_warning = false;
    if (!s_showed_write_warning)
    {
      s_showed_write_warning = true;
      Core::DisplayMessage("\u26A0 Unresolvable write address (ignored)", 4000);
    }
  }
}
// =====================
...
void MMU::GenerateDSIException(u32 effective_address, bool write)
{
  ...
  // Without MMU, silently ignore invalid accesses (reads return 0 at the call site).
  // This matches how real hardware would fault but keeps games running, which is the
  // standard behaviour for the vast majority of titles that poke unmapped low addresses.
  if (!m_system.IsMMUMode())
  {
    static bool s_showed_dsi_warning = false;
    if (!s_showed_dsi_warning)
    {
      s_showed_dsi_warning = true;
      Core::DisplayMessage("\u26A0 Invalid memory access (MMU off)", 4000);
    }
    return;
  }
