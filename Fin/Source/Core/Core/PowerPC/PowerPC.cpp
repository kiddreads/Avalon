...
void PowerPCManager::DoState(PointerWrap& p)
{
  ...
  if (p.IsReadMode())
  {
    if (!m_ppc_state.m_enable_dcache)
    {
      ...
      // Flush wrote dcache contents to memory; invalidate iCache so we don't execute stale
      // instructions that may have been in the dcache and are now in memory.
...
{
#ifdef DOLPHIN_IOS
  // iOS: no JIT support. Force valid core if somehow misconfigured.
  if (cpu_core != CPUCore::Interpreter && cpu_core != CPUCore::CachedInterpreter &&
      cpu_core != CPUCore::InlineCachedInterpreter)
...
{
#ifdef DOLPHIN_IOS
  // iOS: no JIT for App Store compliance.
  static constexpr auto cpu_cores = {
      CPUCore::InlineCachedInterpreter,
