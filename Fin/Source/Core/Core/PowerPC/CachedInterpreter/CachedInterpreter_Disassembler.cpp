...
s32 CachedInterpreter::InlineCmpBc(std::ostream& stream, const InlineCmpBcOperands& operands)
{
  fmt::println(stream,
               "InlineCmpBc(cmp_pc=0x{:08x}, cmp_inst=0x{:08x}, bc_inst=0x{:08x})",
               operands.cmp_pc, operands.cmp_inst.hex, operands.bc_inst.hex);
  return sizeof(AnyCallback) + sizeof(operands);
}
...
#define LOOKUP_KV(...) {AnyCallbackCast(__VA_ARGS__), AnyDisassembleCast(__VA_ARGS__)}
  ...
  static auto sorted_lookup = std::to_array<LookupKV>({
      ...
      LOOKUP_KV(CachedInterpreter::InlineCmpBc),
