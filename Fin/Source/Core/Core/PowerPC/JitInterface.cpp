...
#include "Core/PowerPC/CachedInterpreter/InlineCachedInterpreter.h"
...
// JIT backends are not used on iOS (App Store policy). Exclude from build when building for iOS.
#if defined(_M_X86_64) && !defined(DOLPHIN_IOS)
#include "Core/PowerPC/Jit64/Jit.h"
#endif

#if defined(_M_ARM_64) && !defined(DOLPHIN_IOS)
...
CPUCoreBase* JitInterface::InitJitCore(PowerPC::CPUCore core)
{
  ...
  case PowerPC::CPUCore::InlineCachedInterpreter:
    m_jit = std::make_unique<InlineCachedInterpreter>(m_system);
    break;
