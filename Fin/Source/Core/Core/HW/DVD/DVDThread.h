...
public:
  ...
  // Optional callback when a read completes (e.g. for native video decode on iOS).
  // Called from CPU thread; data is valid only for the duration of the call.
  using OnReadCompleteFn = void (*)(u64 dvd_offset, u32 length, const u8* data, size_t size);
  static OnReadCompleteFn g_on_dvd_read_complete;
