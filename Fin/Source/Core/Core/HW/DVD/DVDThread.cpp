...
namespace DVD
{
DVDThread::OnReadCompleteFn DVDThread::g_on_dvd_read_complete = nullptr;
...
void DVDThread::FinishRead(u64 id, s64 cycles_late)
{
  ...
  {
    if (g_on_dvd_read_complete)
      g_on_dvd_read_complete(request.dvd_offset, request.length, buffer.data(), buffer.size());
