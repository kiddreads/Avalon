...
const Info<int> MAIN_CACHED_INTERPRETER_CODE_REGION_SIZE_MB{{System::Main, "Core",
                                                            "CachedInterpreterCodeRegionSizeMB"},
                                                           128};
...
#ifdef IPHONEOS
// Larger default buffer on iOS to reduce underruns and choppy audio during DVD reads and FMV.
const Info<int> MAIN_AUDIO_BUFFER_SIZE{{System::Main, "Core", "AudioBufferSize"}, 200};
#else
...
#ifdef IPHONEOS
const Info<bool> MAIN_PREFER_SPATIAL_AUDIO{{System::Main, "Core", "PreferSpatialAudio"}, true};
#else
const Info<bool> MAIN_PREFER_SPATIAL_AUDIO{{System::Main, "Core", "PreferSpatialAudio"}, false};
#endif
#ifdef IPHONEOS
const Info<int> MAIN_AUDIO_OUTPUT_MODE{{System::Main, "Core", "AudioOutputMode"}, 1};
const Info<bool> MAIN_AUDIO_UPSAMPLING{{System::Main, "Core", "AudioUpsampling"}, false};
#else
const Info<int> MAIN_AUDIO_OUTPUT_MODE{{System::Main, "Core", "AudioOutputMode"}, 1};
const Info<bool> MAIN_AUDIO_UPSAMPLING{{System::Main, "Core", "AudioUpsampling"}, false};
#endif
...
{
#ifdef IPHONEOS
  // On iOS allow DPL2 with HLE for enhanced stereo/surround matrix decoding.
  return Get(MAIN_DPL2_DECODER);
#else
