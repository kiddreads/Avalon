...
void main()
{
    ...
    // iOS/SDR: always run the perceptual mapping, even when HDR/linear flags are off.
    // Guard against zero/invalid paper white so we don't blow up.
    float hdr_paper_white = hdr_paper_white_nits / hdr_sdr_white_nits;
    if (!(hdr_paper_white > 0.0))
        hdr_paper_white = 1.0;
