...
- (BOOL)hasFadeTruncating {
    return self.fadeTruncatingMode != ShadowLabelFadeTruncatingModeNone;
}
...
- (CGFloat)strokeSizeDependentOnStrokePosition {
    switch (self.strokePosition) {
        case ShadowLabelStrokePositionCenter:
...
- (void)drawRect:(CGRect)rect {
    ...
    BOOL needsMask = hasGradient || (hasStroke && self.strokePosition == ShadowLabelStrokePositionInside) || hasInnerShadow;
    ...
    if (hasStroke) {
        ...
        if (self.strokePosition == ShadowLabelStrokePositionOutside) {
            // Create an image from the text.
            image = CGBitmapContextCreateImage(context);
        } else if (self.strokePosition == ShadowLabelStrokePositionInside) {
            // Clip the current context to alpha mask.
            CGContextClipToMask(context, rect, alphaMask);
        }
    ...
    if (hasFadeTruncating) {
        ...
        CGImageRef linearGradientImage = [self linearGradientImageWithRect:rect fadeHead:self.fadeTruncatingMode & ShadowLabelFadeTruncatingModeHead fadeTail:self.fadeTruncatingMode & ShadowLabelFadeTruncatingModeTail];
...
- (UIEdgeInsets)fittingTextInsets {
    ...
    if (hasStroke) {
        switch (self.strokePosition) {
            case ShadowLabelStrokePositionOutside:
