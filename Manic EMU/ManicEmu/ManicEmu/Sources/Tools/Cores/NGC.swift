...
extension GameType {
    static let ngc = GameType("public.aoshuang.game.ngc")
}

@objc enum NGCGameInput: Int, Input, CaseIterable {
    case a
    case b
    case x
    case y
...
struct NGC: DeltaCoreProtocol {
    static let core = NGC()
    
    var name: String { "NGC" }
    var identifier: String { "com.aoshuang.NGCCore" }
    
    var gameType: GameType { GameType.ngc }
    var gameInputType: Input.Type { NGCGameInput.self }
    var allInputs: [Input] { NGCGameInput.allCases }
    ...
    var supportedCheatFormats: Set<CheatFormat> {
        ...
        let gecko = CheatFormat(name: "Gecko",
                                format: "XXXXXXXX YYYYYYYY",
                                type: .Gecko)
        let armax = CheatFormat(name: "ARMax",
                                format: "XXXX-YYYY-ZZZZZ",
                                type: .ARMax)
        return [actionReplay, gecko, armax]
    }
    
    var emulatorBridge: EmulatorBridging { NGCEmulatorBridge.shared }
...
Log.debug("🎮 \(objectInfo(self)) 点击了:\(gameInput)")
    ...
    func gameInputToCoreInput(gameInput: NGCGameInput) -> LibretroButton? {
