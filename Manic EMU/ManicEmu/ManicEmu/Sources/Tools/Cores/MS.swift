...
struct MS: DeltaCoreProtocol {
    
    static let core = MS()
    
    var name: String { "MS" }
    var identifier: String { "com.aoshuang.MSCore" }
    
    var gameType: GameType { GameType.ms }
    var gameInputType: Input.Type { MSGameInput.self }
    ...
    var emulatorBridge: EmulatorBridging { MSEmulatorBridge.shared }
...
class MSEmulatorBridge : EmulatorBridgeBase {
