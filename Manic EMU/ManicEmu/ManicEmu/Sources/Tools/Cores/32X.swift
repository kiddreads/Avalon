...
struct S2X: DeltaCoreProtocol {
    static var isJGenesisCore: Bool = false
    
    static let core = S2X()
    
    var name: String { "32X" }
    var identifier: String { "com.aoshuang.32XCore" }
    
    var gameType: GameType { GameType._32x }
    var gameInputType: Input.Type { S2XGameInput.self }
    ...
    var emulatorBridge: EmulatorBridging { S2XEmulatorBridge.shared }
...
class S2XEmulatorBridge : EmulatorBridgeBase {
