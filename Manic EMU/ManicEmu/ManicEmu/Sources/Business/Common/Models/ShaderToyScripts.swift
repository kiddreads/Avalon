...
extension ShaderToy {
    ...
    static let ToonCloud = """
        ...
        void mainImage( out vec4 fragColor, in vec2 fragCoord )
        {
            ...
            for(float J=0.0; J<=1.0; J+=1.0/3.0)
            {
                // Cloud Layer: 
    ...
    // BigWings — restore depth/trees; keep stars slightly cheaper than original
    static let OverTheMoon = """
        #define PI 3.1415
        ...
        float skewbox(vec2 uv, vec3 top, vec3 bottom, float blur) {
            ...
            return B(left, right, uv.x, blur) * B(bottom.z, top.z, uv.y, blur);
        }
        ...
        vec4 pine(vec2 uv, float focus) {
            ...
            return vec4(mix(vec3(1.), vec3(.8), shadow), c);
        }
        ...
        vec4 landscape(vec2 uv, float d, float p, float f, float a, float y, float seed, float focus) {
            ...
            float e = max(fwidth(uv.y), 0.0015);
            ...
            float ty = within(h, h+treeHeight, uv.y);
            x += (n-.5)*.6;
            // Soften pine AA at low res
            vec4 pineCol = pine(vec2(x, ty/d), focus + d*.1 + e*2.);
        ...
        void mainImage(out vec4 fragColor, in vec2 fragCoord)
        {
            ...
            vec4 col = vec4(1.-length(MOONPOS-bgUV)/1.4)*.8;
            float moonC = circ(bgUV, MOONPOS, .07, .001);
            moonC *= 1.-circ(bgUV, MOONPOS+vec2(.03), .07, .001)*.95;
            col += vec4(saturate(moonC)*.8);
            col += stars(uv, t);
            ...
            float amplitude = .02;
            vec4 trees = vec4(0.);
            // 8 layers — denser than 5, cheaper than original 10
            for (float i = 0.; i < 8.; i++) {
                vec4 layer = landscape(uv, dist, t+i, 3., amplitude, height, i, .01);
                layer.rgb *= mix(vec3(.1, .1, .2), vec3(.3)+col.x, 1.-i/8.);
                trees = mix(trees, layer, layer.a);
                dist -= .12;
                height -= .07;
            }
            col = mix(col, trees, trees.a);
            col += vec4(circ(bgUV, MOONPOS, .1, .2)*.2);
            col = saturate(col);
